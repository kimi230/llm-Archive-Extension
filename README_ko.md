...
### 4️⃣ 고급 기능: 내용 정리 vs 아카이빙

사이드패널의 `내용정리` 스위치를 통해 저장 방식을 선택할 수 있습니다.

#### ⚫️ 내용정리 OFF (기본값)
- **목적**: 대화 내용의 **순수 아카이빙**
- **작동 방식**: 대화 내용을 있는 그대로(`user`, `assistant` 턴 유지) 하나의 마크다운 파일로 저장합니다.
- **특징**: 
    - AI 재가공 없이 원본을 보존합니다.
    - **참고**: 이 모드에서도 제목 표시줄 옆의 **`AI` 버튼**을 누르면, Gemini/GPT 모델을 사용하여 **제목, 태그, 요약**만 자동으로 생성할 수 있습니다.

#### 🔵 내용정리 ON (AI 모드)
- **목적**: 대화를 분석하여 **구조화된 지식 문서**로 변환
- **필수 조건**: OpenAI API Key 설정 필요
- **작동 방식**:
    1. **Planning Agent**: 대화 전체를 분석하여 주제를 파악하고, 단일 또는 다중 문서로 분할할지 계획을 수립합니다.
    2. **추출 계획 확인**: 사용자에게 생성될 폴더명과 문서 목록을 미리 보여주고 승인을 받습니다.
    3. **Writing Agent**: 승인된 계획에 따라 대화체를 명확한 **기술 문서 스타일**로 다시 작성(Rewriting)하여 저장합니다.
- **결과물**: 선택된 경로 하위에 새로운 폴더가 생성되고, 그 안에 정리된 여러 개의 파일이 저장됩니다.
